...
# CJK Unicode ranges that are double-width
_CJK_RANGES = (
    (0x1100, 0x115F), (0x2E80, 0x303E), (0x3040, 0xA4CF),
...
class VibrationDetector:
    ...
    def _classify(self, detections, t, amp):
        ...
        if ns >= 4 and amp > 0.05:
            sev, sym, lbl = 'CHOC_MAJEUR', '★', '重大冲击'
        elif ns >= 3 and amp > 0.02:
            sev, sym, lbl = 'CHOC_MOYEN', '▲', '冲击'
        elif 'PEAK' in sources and amp > 0.005:
            sev, sym, lbl = 'MICRO_CHOC', '△', '微冲击'
        elif ('STA/LTA' in sources or 'CUSUM' in sources) and amp > 0.003:
            sev, sym, lbl = 'VIBRATION', '●', '振动'
        elif amp > 0.001:
            sev, sym, lbl = 'VIB_LEGERE', '○', '轻微振动'
        else:
            sev, sym, lbl = 'MICRO_VIB', '·', '微振动'
...
def _vlen(s):
    ...
    return sum(2 if _is_wide(c) else 1 for c in clean)


def _cjk_ljust(s, width):
    """Left-justify s to display-width `width`, accounting for CJK double-width chars."""
    dw = sum(2 if _is_wide(c) else 1 for c in s)
...
def _sep(label=''):
    if label:
        ...
        return f"{DIM}├─{label}{'─' * max(0, rest)}┤{RST}"
    return f"{DIM}├{'─' * W}┤{RST}"
...
def render(det, t_start, restarts, kbflash=None,
    ...
    if has_dwt:
        ...
        msg = 'pip install PyWavelets' if not det._dwt_ok else '累积中...'
    ...
    if det.period is not None and det.period_cv is not None and det.period_cv < 0.5:
        ...
        a(_line(f" 周期:{det.period:.3f}秒 ±{det.period_std:.3f}"
                f"  频率:{det.period_freq:.2f}Hz  规律性:{reg}%"))
        syms = ''.join(f"──{e['sym']}" for e in list(det.events)[-12:])
        a(_line(f" {DIM}{syms}──{RST}"))
    else:
        a(_line(f" {DIM}未检测到规律性模式{RST}"))
        a(_line(''))
    ...
    if hr_active:
        ...
        a(_line(f" {DIM}未检测到心跳（将手腕搭在笔记本键盘附近）{RST}"))
        a(_line(''))
    ...
    gw = W - 18
    a(_line(f' {DIM}滚转{RST} {CYN}{_gauge(roll_d, -180, 180, gw)}{RST} {roll_d:>+7.1f}°'))
    a(_line(f' {DIM}俯仰{RST} {CYN}{_gauge(pitch_d, -90, 90, gw)}{RST} {pitch_d:>+7.1f}°'))
    a(_line(f' {DIM}偏航{RST} {CYN}{_gauge(yaw_d, -180, 180, gw)}{RST} {yaw_d:>+7.1f}°'))
    ...
    for ev in reversed(recent):
        ...
        a(_line(f" {DIM}{ev['tstr']}{RST} {c}{ev['sym']} {_cjk_ljust(ev['lbl'], 8)}{RST}"
                f" {ev['amp']:.5f}g {bands}"))
    ...
    a(_line(f" {DIM}Ctrl+C 保存并退出{RST}"))
    a(f"{DIM}└{'─' * W}┘{RST}")
...
def main():
    ...
    try:
        ...
        print(f"{DIM}[*] 正在保存 {len(det.events)} 个事件到 {logpath}{RST}")
        obj = {
        ...
        print(f"{DIM}[ok] {det.sample_count} 个样本，"
              f"{restart_count[0]} 次重启{RST}")
